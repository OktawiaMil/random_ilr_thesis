...
knitr:
  opts_chunk:
    fig.out: density_impact_files/graphics/
    fig.width: 15
    fig.height: 12
---
...
#| message: false
#| warning: false
# Create/clean graphics directory (absolute), set fig.path (relative)
graphics_abs <- here::here("quatro_scripts", "density_impact_files", "graphics")
if (dir.exists(graphics_abs)) {
  unlink(graphics_abs, recursive = TRUE, force = TRUE)
}
dir.create(graphics_abs, recursive = TRUE, showWarnings = FALSE)
knitr::opts_chunk$set(fig.path = "density_impact_files/graphics/")
...
density_impact <- function(seed_start = 2025, n_org_features, density, top_k, n_iters = 10){
  ...
  for(i in 1:n_iters){
    ...
    out <- understand_randilr(ghl = ghl_mat, 
...
results <- tibble(
  ...
  mutate(
    ghl_analysis = map(density, ~density_impact(seed_start = seed_start,
...
results|> 
  ...
  mutate(feature = str_remove(feature, "org_"),
