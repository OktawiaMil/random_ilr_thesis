...
**Reminder**: Density $\delta=0.1$ means that $10\%$ of the off-diagonal entries in the skew-symmetric matrix $S$ are non-zero; each such non-zero value is drawn from a standard normal distribution $(s_{ij}\sim\mathcal{N}(0,1) \; \text{and} \; s_{ji}=-s_{ij})$. The **unit density** refers to the very sparse case in which exactly one off-diagonal element in the lower triangle and its mirror element in the upper triangle are non-zero; all other off-diagonal entries are zero.
...
-   analyzed density of skew-symmetric matrix : unit density (very sparse skew-symmetric matrix) and $\delta \in (0.01, 0.02, \dots, 0.1, 0.2, 0.3, \dots, 0.9)$,
...
data_plt1 <- results |>
    ...
    mutate(
        ...
        density = if_else(is.na(density), "Unit", as.character(density)),
        density = factor(density,
            levels = c("Unit", sort(as.numeric(setdiff(unique(density), "Unit"))))
        ),
        part = str_to_title(part)
    )

data_plt1 |>
    filter(density %in% c("Unit", seq(0.01, 0.09, by = 0.01))) |>
    ggplot(aes(x = feature, y = value, fill = part)) +
    geom_bar(position = "dodge", stat = "identity") +
    scale_fill_manual(values = c(Numerator = "#2b83ba", Denominator = "#d7191c")) +
...
data_plt1 |>
    filter(density %in% seq(0.1, 0.9, by = 0.1)) |>
    ggplot(aes(x = feature, y = value, fill = part)) +
    geom_bar(position = "dodge", stat = "identity") +
    scale_fill_manual(values = c(Numerator = "#2b83ba", Denominator = "#d7191c")) +
    facet_wrap(~density, ncol = 3) +
